class MaxHeap {
    ...
    // metodo para eliminar el máximo
    ...
    //  método para listar nodos dentro de un rango
    public listInRange(min: number, max: number): number[] {
        const nodesInRange: number[] = [];
        for (let i = 1; i <= this.n; i++) {
            if (this.heap[i] >= min && this.heap[i] <= max) {
...
console.log("Números en el rango 3 a 6:", myHeap.listInRange(3, 6)); 